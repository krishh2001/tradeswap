@extends('admin.layouts.admin')

@section('content')
    {{-- DataTables CSS --}}
    <link rel="stylesheet" href="https://cdn.datatables.net/1.13.6/css/jquery.dataTables.min.css">

    {{-- jQuery & DataTables JS --}}
    <script src="https://code.jquery.com/jquery-3.7.1.min.js"></script>
    <script src="https://cdn.datatables.net/1.13.6/js/jquery.dataTables.min.js"></script>

    <style>
        /* Ensure modal works correctly */
        .modal {
            display: none;
            position: fixed;
            z-index: 9999;
            background-color: rgba(0, 0, 0, 0.5);
            top: 0;
            left: 0;
            width: 100%;
            height: 100%;
            justify-content: center;
            align-items: center;
        }

        .modal.show {
            display: flex;
        }
    </style>

    <div class="user-dashboard">
        <div class="search-create-row">
            <div class="search-box">
                <svg xmlns="http://www.w3.org/2000/svg" class="search-icon" viewBox="0 0 24 24" fill="none"
                    stroke="currentColor">
                    <circle cx="11" cy="11" r="8" stroke-width="2"></circle>
                    <line x1="21" y1="21" x2="16.65" y2="16.65" stroke-width="2"></line>
                </svg>
                <input type="text" id="couponSearch" placeholder="Search by coupon code...">
            </div>
            <a href="{{ route('admin.coupons.create') }}" class="user-create-btn">+ Create</a>
        </div>

        <div class="support-table-wrapper">
            <table class="support-table" id="couponTable">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Coupon Code</th>
                        <th>Discount</th>
                        <th>Usage Limit</th>
                        <th>Limit (Days)</th>
                        <th>Used</th>
                        <th>Expiry Date</th>
                        <th>Status</th>
                        <th>Actions</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($coupons as $index => $coupon)
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td>{{ $coupon->code }}</td>
                            <td>{{ $coupon->discount }}%</td>
                            <td>{{ $coupon->usage_limit }}</td>
                            <td>{{ $coupon->limit_days }}</td>
                            <td>{{ $coupon->used }}</td>
                            <td>{{ $coupon->expiry_date }}</td>
                            <td>
                                <button
                                    class="status-toggle-btn {{ $coupon->status === 'active' ? 'badge-success' : 'badge-danger' }}"
                                    data-id="{{ $coupon->id }}" data-status="{{ $coupon->status }}">
                                    {{ ucfirst($coupon->status) }}
                                </button>
                            </td>
                            <td>
                                <a href="{{ route('admin.coupons.view', $coupon->id) }}"
                                    class="btn-small btn-view">View</a>
                                <a href="{{ route('admin.coupons.edit', $coupon->id) }}"
                                    class="btn-small btn-edit">Edit</a>
                                <button class="btn-small btn-delete"
                                    onclick="openDeleteModal({{ $coupon->id }})">Delete</button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
    </div>

    <!-- Delete Modal -->
    <div class="modal" id="sliderDeleteModal">
        <div class="modal-content">
            <button class="modal-close" onclick="closeDeleteModal()">&times;</button>
            <h3>Confirm Delete</h3>
            <p>Are you sure you want to delete this coupon?</p>
            <form id="deleteProductForm" method="POST" action="">
                @csrf
                @method('DELETE')
                <div class="modal-actions">
                    <button type="button" class="cancel" onclick="closeDeleteModal()">Cancel</button>
                    <button type="submit" class="confirm">Yes, Delete</button>
                </div>
            </form>
        </div>
    </div>

    <script>
        let couponTable;

        $(document).ready(function() {
            // Initialize DataTable
            couponTable = $('#couponTable').DataTable({
                paging: true,
                info: true,
                searching: true, // ✅ must remain true for custom search to work
                lengthChange: false,
                order: [],
                columnDefs: [{
                    orderable: false,
                    targets: [7, 8]
                }],
                dom: 'lrtip' // ✅ removes default search box & "show entries"
            });


            // Custom Search Input
            $('#couponSearch').on('keyup', function() {
                couponTable.search(this.value).draw();
            });

            // Toggle Status
            $('.status-toggle-btn').on('click', function() {
                const button = $(this);
                const couponId = button.data('id');
                const currentStatus = button.data('status');
                const newStatus = currentStatus === 'active' ? 'expired' : 'active';

                fetch(`/admin/coupons/${couponId}/toggle-status`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        },
                        body: JSON.stringify({
                            status: newStatus
                        })
                    })
                    .then(res => res.json())
                    .then(data => {
                        if (data.success) {
                            button.text(newStatus.charAt(0).toUpperCase() + newStatus.slice(1));
                            button.data('status', newStatus);
                            button.toggleClass('badge-success badge-danger');
                        } else {
                            alert('Failed to update status.');
                        }
                    })
                    .catch(() => alert('Request failed.'));
            });
        });

        function openDeleteModal(id) {
            document.getElementById('deleteProductForm').action = `/admin/coupons/${id}`;
            document.getElementById('sliderDeleteModal').classList.add('show');
        }

        function closeDeleteModal() {
            document.getElementById('sliderDeleteModal').classList.remove('show');
        }
    </script>
@endsection
